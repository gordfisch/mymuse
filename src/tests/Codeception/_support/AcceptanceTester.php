...
class AcceptanceTester extends Actor
{
	...
	*/
	...
	public function updateAmplitudePlugin () 
	{
		$this->comment('Update Plugin Amplitude');
		$this->amOnPage('administrator/index.php?option=com_plugins&view=plugins');
		$this->searchForItem('Audio Amplitude');

		$this->waitForElement("//a[contains(text(), 'Audio Amplitude')]", 30);
		$this->click("//a[contains(text(), 'Audio Amplitude')]");
		$this->cats['MyMuse'] = 87;
		$this->mymuseSelectOptionByName('jform[params][mycategories][]', $this->cats['MyMuse']);

		$this->click(['css' => 'button.button-save']);
		$this->wait(2);
		$this->see('saved');
	...
	*/
	...
	public function createMymuseProduct($mock)
	{
		...
		$top_folder = dirname(dirname(dirname(dirname(dirname(__FILE__)))));

		$src = $top_folder."/mymuse-images";
		$dst = $joomla_folder."/images/mymuse";

		if(!file_exists($dst)){
			if(!mkdir($dst, 0755)){
				$this->comment('Could not make dir '.$dst);
				return false;
			}
			$this->custom_copy($src, $dst);
		}
		if(!file_exists($joomla_folder.'/'.$mock->jform_list_image)){
	...
	*/
	 public function createMymuseTrack($mock, $config)
	 {
		...
		if($config['my_copy_tracks']){
			...
			$from_dir	 = $top_folder.'/mymuse-downloads';
	...
	*/
	public function changeGlobalOptions($mock)
	{
		...
		foreach($mock->select as $select){
			if($select['type'] == "select"){
				$this->selectOptionInSelectById($select['option'], $select['value']);
				$this->wait(1);	 
			}elseif($select['type'] == "radio"){
				$this->comment('Looking for SwitcherRadioField "'.$select['option'].'"');
				$this->mymuseSelectOptionInSwitcherRadioField($select['option']);
				//$this->selectOptionInRadioFieldById($select['option'], $select['value']);
	...
	*/
	...
	/**
	 * Selects an option in a Selector based on its id
	 *
	 ...
	 * @return void
	 */
	public function selectOptionInSelectById($selectId, $option)
	{

		$this->click(['css' => "#".$selectId]);
		$this->click(['xpath' => "//select[@id='$selectId']//option[text()='$option']"]);
	...
	public function clearMenus()
	{
		...
		{
			$this->comment("Check all does not exist on Trashed page, skipping");
			return;
		}
		$this->click(["xpath" => "//input[@name='checkall-toggle']"]);
		...
		//$this->selectOption('#filter_published', 'Published');
	...
	*/
	 ...
	 * @throws  \Exception
	 */
	public function mymuseDisablePlugin($pluginName)
	{
		$this->amOnPage('/administrator/index.php?option=com_plugins');
		$this->comment('I check for Notices and Warnings');
		$this->checkForPhpNoticesOrWarnings();
		$this->searchForItem($pluginName);

		$this->waitForElement("//a[contains(text(), '" . $pluginName . "')]", 30);

		if($this->seePageHasElement(['.row0 a.tbody-icon span.icon-publish'])){
			$this->click(array('css' => ".row0 a.tbody-icon span.icon-publish"));
			$this->see(' disabled', '#system-message-container');
			return true;
		}
		else
		{
			$this->comment('Plugin already disabled.');
		}
	 ...
	 * @throws Exception
	 */
	public function mymuseEnablePlugin($pluginName)
	{
		$this->amOnPage('/administrator/index.php?option=com_plugins');
		$this->comment('I check for Notices and Warnings');
		$this->checkForPhpNoticesOrWarnings();
		...
		{
			$this->comment('Plugin already enabled.');
		}
	}
...
/* SELECTOR FUNCTIONS */
	 ...
	 * @since   4.0.0
	 */
	public function mymuseSelectOptionInSwitcherRadioField($selector)
	{
		$isChecked = $this->grabAttributeFrom(['id' => $selector], 'checked');
		$this->comment('isChecked attribute was '.$isChecked );
		if ($isChecked == 'true')
		{
			$this->comment('The value seems to already be checked.');
		}
		else
		{
			$this->click(['id' => $selector]);
		}
	}
	 ...
	 * @since   4.0.0
	 */
	public function mymuseSelectOptionByName($name, $value)
	{

		$this->comment('Select with name : select[name="' . $name . '"]' );
		//if( $this->seePageHasElement(['select[name="' . $name . '"]']) ){

		//select[name="jform[params][mycategories][]"] > option[value="88"]
			//$this->comment('Select with name-and-value : select[name="' . $name . '"] > option[value="' . $value . '"]' );
			//$this->click(['select[name="' . $name . '"] > option[value="' . $value . '"]']);
		//}else{
		//	$this->comment('Could not find element with name: '.$name);
		//}

		$selector = 'select[name="' . $name . '"]';
		$this->selectOption($selector, "- MyMuse");
