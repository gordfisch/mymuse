 ...
 * @since  1.6.0
 */
class MymuseController extends FormController
{
	...
	protected function allowAdd($data = array())
	{
		...
		if ($categoryId)
		{
			...
			$allow = $user->authorise('core.create', 'com_mymuse.category.' . $categoryId);
		}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($user->authorise('core.edit', 'com_mymuse.product.' . $recordId))
		{
			return true;
		}

		// Check edit own on the record asset (explicit or inherited)
		if ($user->authorise('core.edit.own', 'com_mymuse.product.' . $recordId))
		{
	...
	public function cancel($key = 'a_id')
	{
		...
		if ($customCancelRedir)
		{
			...
			{
				// Redirect to the same product submission form (clean form).
	...
	public function save($key = null, $urlVar = 'a_id')
	{
		...
		$app       = Factory::getApplication();
		$productId = $app->input->getInt('a_id');

		// Load the parameters.
		$params   = $app->getParams();
		$menuitem = (int) $params->get('redirect_menuitem');

		// Check for redirection after submission when creating a new product only
		if ($menuitem > 0 && $productId == 0)
		{
			$lang = '';
	...
	public function vote()
	{
		...
		if ($user_rating > -1)
		{
			...
			{
				$this->setRedirect($url, Text::_('COM_MYMUSE_PRODUCT_VOTE_SUCCESS'));
			}
			else
			{
				$this->setRedirect($url, Text::_('COM_MYMUSE_PRODUCT_ARTICLE_VOTE_FAILURE'));
			}
		}
	}

	public function ajaxtogglecart()
	{
