...
class CreateZipHelper {
    ...
    function create_zip($path, $files = array(),$destination = '',$overwrite = false) {
        ...
        if(is_array($files)) {
            ...
            foreach($files as $i => $file) {
                ...
                if(file_exists($path.$file)) {
                    ...
                    $valid_files[$i]['name'] = $file;
                    $valid_files[$i]['data'] = file_get_contents($path.$file);
                }
            }
        }
        ...
        if(count($valid_files)) {
            ...
            $zip->create($destination, $valid_files );
...
/*
            ...
            if($zip->open($destination,$overwrite ? ZIPARCHIVE::OVERWRITE : ZIPARCHIVE::CREATE) !== true) {
                //echo $destination.' '.$zip->open($destination,$overwrite ? ZIPARCHIVE::OVERWRITE : ZIPARCHIVE::CREATE); exit;
                return false;
            }
            //add the files
