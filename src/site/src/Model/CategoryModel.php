...
class CategoryModel extends ListModel
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$value = $this->getUserStateFromRequest('com_mymuse.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
		$this->setState('filter.tag', $value);

		// Optional filter text
		$search = $app->getUserStateFromRequest('com_mymuse.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
		...
		{
			$limit = $app->getUserStateFromRequest('com_mymuse.category.list.' . $itemid . '.limit', 'limit', $params->get('display_num'), 'uint');
		}
	...
	public function getItems()
	{
		...
		if ($this->_products === null && $category = $this->getCategory())
		{
			...
			$model->setState('filter.artist_id', $category->id);
			$model->setState('filter.published', $this->getState('filter.published'));
			$model->setState('filter.access', $this->getState('filter.access'));
			$model->setState('filter.language', $this->getState('filter.language'));
			$model->setState('filter.featured', $this->getState('filter.featured'));
			$model->setState('list.ordering', $this->_buildMymuseOrderBy());
	 ...
	 * @since   1.5
	 */
	protected function _buildMymuseOrderBy()
	{
		$app       = Factory::getApplication();
		$db        = $this->getDbo();
