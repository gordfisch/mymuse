...
/**
 * Mymuse Component Article Model
 *
 * @since  1.5
 */
class FormModel extends \Joomla\Component\Mymuse\Administrator\Model\ArticleModel
{
	/**
	...
	protected function populateState()
	{
		...
		$this->setState('product.catid', $app->input->getInt('catid', $catId));
	 ...
	 *
	 * @return  mixed  Mymuse item data object on success, false on failure.
	 */
	public function getItem($itemId = null)
	{
		...
		else
		{
			...
			if ($catId)
			{
				$value->params->set('access-change', $user->authorise('core.edit.state', 'com_mymuse.category.' . $catId));
				$value->catid = $catId;
			}
			else
			{
				$value->params->set('access-change', $user->authorise('core.edit.state', 'com_mymuse'));
			}
		}

		$value->producttext = $value->introtext;

		if (!empty($value->fulltext))
		{
			$value->producttext .= '<hr id="system-readmore">' . $value->fulltext;
		}
		...
		if ($itemId)
		{
			...
			$value->tags->getTagIds($value->id, 'com_mymuse.product');
	...
	public function save($data)
	{
		...
		if (Associations::isEnabled() && !empty($data['id'])
			&& $associations = Associations::getAssociations('com_mymuse', '#__content', 'com_mymuse.item', $data['id']))
		{
	...
	public function getForm($data = [], $loadData = true)
	{
		...
		// On edit product, we get ID of product from product.id state, but on save, we use data from input
		$id = (int) $this->getState('product.id', $app->input->getInt('a_id'));

		// Existing record. We can't edit the category in frontend if not edit.state.
		if ($id > 0 && !$user->authorise('core.edit.state', 'com_mymuse.product.' . $id))
		{
		...
		if ($this->getState('product.id') && Associations::isEnabled())
		{
			$associations = Associations::getAssociations('com_mymuse', '#__content', 'com_mymuse.item', $id);
