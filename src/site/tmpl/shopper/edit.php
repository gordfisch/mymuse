...
<?php if($params->get('my_registration') == "full" || $params->get('my_registration') == "joomla"){ ?>
            ...
            if(that.username != null && that.username.value == ""){
                    alert("<?php echo Text::_('MYMUSE_USERNAME_IS_REQUIRED') ?>");
                    return false;
            }

            //check for password matches
            if(that.password.value != that.password2.value){
                    alert("<?php echo Text::_('MYMUSE_PASSWORDS_DO_NOT_MATCH') ?>");
                    return false;
            }
...
<?php } ?>
            ...
            if(that.email != null && that.email.value == ""){
                    alert("<?php echo Text::_('MYMUSE_EMAIL_IS_REQUIRED') ?>");
                    return false;
            }
            var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;

            if(reg.test(that.email.value) == false) {
               alert('<?php echo Text::_('MYMUSE_EMAIL_INVALID') ?>');
               return false;
            }
                     
            //check for first_name
            if(that.first_name != null && that.first_name.value == ""){
                    alert("<?php echo Text::_('MYMUSE_FIRST_NAME_IS_REQUIRED') ?>");
                    return false;
            }
            //check for last_name
            if(that.last_name != null && that.last_name.value == ""){
                    alert("<?php echo Text::_('MYMUSE_LAST_NAME_IS_REQUIRED') ?>");
                    return false;
            }
            //check for country
            if(that.country != null && that.country.value == "0"){
                    alert("<?php echo Text::_('MYMUSE_COUNTRY_IS_REQUIRED') ?>");
                    return false;
            }
<?php if($params->get('my_state_required')){ ?>
            //check for state
            if(that.state != null && that.state.value == "0"){
                    alert("<?php echo Text::_('MYMUSE_STATE/PROVINCE_IS_REQUIRED') ?>");
                    return false;
            }
<?php } ?>
<?php if($params->get('my_registration') == "full"){ ?>
            ...
            if(that.address_1 != null && that.address_1.value == ""){
                    alert("<?php echo Text::_('MYMUSE_ADDRESS_IS_REQUIRED') ?>");
                    return false;
            }
            //check for city
            if(that.city != null && that.city.value == ""){
                    alert("<?php echo Text::_('MYMUSE_CITY_IS_REQUIRED') ?>");
                    return false;
            }

            //check for zip
            if(that.zip != null && that.zip.value == ""){
                    alert("<?php echo Text::_('MYMUSE_ZIP/POSTAL_CODE_IS_REQUIRED') ?>");
                    return false;
            }
...
<?php if($params->get('my_termsofservice')){ ?>
			...
			if(that.terms_of_service != null && that.terms_of_service.checked == false){
        		alert("<?php echo Text::_('MYMUSE_TERMSOFSERVICE_REQUIRED') ?>");
        		return false;
			}
...
<?php } ?>
        ...
        <h2 class="my-title"><?php echo Text::_('MYMUSE_EDIT_PROFILE') ?></h2>
		<div class="message"><?php echo Text::_('MYMUSE_MAKE_CHANGES_AND_SAVE') ?></div>
		
		
		<table cellpadding="3" cellspacing="0" border="0" width="100%" class="contentpane">
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_FIRST_NAME'); ?>:<span class="mymuse_msg">*</span></td>
				<td><input class="inputbox" CLASS="submit" type="text" name=first_name
					maxlength="32" size="22" value="<?php echo $shopper->first_name ?>" /></td>
			</tr>
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_LAST_NAME'); ?>:<span class="mymuse_msg">*</span></td>
...
<?php if($params->get('my_registration') == "full" || $params->get('my_registration') == "joomla"){ ?>
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_USERNAME'); ?>:<span class="mymuse_msg">*</span></td>
				<td width=400><input class="inputbox" type="text" name="username"
					maxlength="32" size="22" value="<?php echo $user->username ?>" /></td>
			</tr>
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_PASSWORD'); ?>:<span class="mymuse_msg">*</span></td>
				<td><input class="inputbox" type="password" name="password"
					MAXLENGTH="32" SIZE="22" value="" /></td>
			</tr>
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_CONFIRM_PASSWORD'); ?>:<span class="mymuse_msg">*</span></td>
...
<?php } ?>
			...
			<tr>
				<td align=right nowrap><?php echo Text::_( 'MYMUSE_COUNTRY' ) ?>:<span class="mymuse_msg">*</span></td>
				<td><?php  echo $lists['country']; ?></td>
			</tr>
			<tr>
				<td align=right nowrap><?php echo Text::_( 'MYMUSE_STATE/PROVINCE' ) ?>:<span class="mymuse_msg">*</span></td>
...
<?php if($params->get('my_termsofservice') == "1"){ ?>
			<tr>
				<td align=right nowrap><?php echo Text::_('MYMUSE_ACCEPT_TERMSOFSERVICE'); ?> 
				<?php if($params->get('my_termsofservice_link') != ""){ ?>
					<a href="<?php echo $params->get('my_termsofservice_link'); ?>">
				<?php } ?>
				<?php echo Text::_('MYMUSE_TERMS_OF_SERVICE'); ?>
				<?php if($params->get('my_termsofservice_link') != ""){ ?>
				</a>
				<?php } ?> :
				</td>
...
<?php } ?>			
            ...
            <tr>
            	<td>* required</td>
            	<td><input class="button" type="submit" name="Register" value="<?php echo Text::_('MYMUSE_SAVE') ?>" /></td>
            </tr>
