...
<div class="row-fluid item-row">
	...
	<div class="span4 col-md-4">
			...
			<?php echo JHtml::_('string.truncate', strip_tags($product->introtext), 200, true); ?>
			<?php endif;
			...
			<?php if ($this->params->get('show_readmore') && $product->readmore) :
				if ($product->access) :
					$link = JRoute::_(MyMuseHelperRoute::getProductRoute($product->slug, $product->catid));
				else :
					$menu = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$itemId = $active->id;
					$link1 = JRoute::_('index.php?option=com_users&view=login&Itemid=' . $itemId);
					$returnURL = JRoute::_(MyMuseHelperRoute::getProductRoute($product->slug, $product->catid));
					$link = new JURI($link1);
