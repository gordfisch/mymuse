 ...
 * Mymuse helper.
 */
class MymuseUpdate extends ContentHelper
{
	...
	function makeProduct($data)
	{
		...
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);     // follow redirects
		curl_setopt($ch, CURLOPT_ENCODING,"");       // handle all encodings
		curl_setopt($ch, CURLOPT_USERAGENT, "mymuse"); // who am i
		curl_setopt($ch, CURLOPT_AUTOREFERER, true);     // set referer on redirect
		curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 120);      // timeout on connect
		curl_setopt($ch, CURLOPT_TIMEOUT, 120);      // timeout on response
	...
	function makeProductObject($p)
	{
		...
		if($id = $db->loadResult()){
			...
			$this->error =  "Could not make product ".MymuseHelper::print_pre($_POST,true);
			return false;
		}
	}
	...
	function get_data($url, $file)
	{
		...
		if($params->get('storage', 'regular') == 'amazon' ){
			...
			$GLOBALS['mymuseStorage']->fileUpload($tmpName, $file);
		}else{
