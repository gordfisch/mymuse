...
class Mp3fileHelper
{
    ...
    protected function readmp3frame()
    {
        ...
        if ($iscbrmp3)
        {
            ...
            $this->mp3data['Bitrate'          ] = Mp3fileHelper::bitratelookup($this->mp3data);
            $this->mp3data['Sampling Rate'    ] = Mp3fileHelper::samplelookup($this->mp3data);
            $this->mp3data['Frame Size'       ] = Mp3fileHelper::getframesize($this->mp3data);
            $this->mp3data['Length'           ] = Mp3fileHelper::getduration($this->mp3data,$this->tell2());
            $this->mp3data['Length mm:ss'     ] = Mp3fileHelper::seconds_to_mmss($this->mp3data['Length']);
...
/*               ||
    ...
    public static function bitratelookup(&$mp3)
    {
        ...
        $whichcolumn=-1;
        if      (Mp3fileHelper::is_mpeg10($mp3) && Mp3fileHelper::is_layer1($mp3) )//V1,L1
            $whichcolumn=0;
        else if (Mp3fileHelper::is_mpeg10($mp3) && Mp3fileHelper::is_layer2($mp3) )//V1,L2
            $whichcolumn=1;
        else if (Mp3fileHelper::is_mpeg10($mp3) && Mp3fileHelper::is_layer3($mp3) )//V1,L3
            $whichcolumn=2;
        else if (Mp3fileHelper::is_mpeg20or25($mp3) && Mp3fileHelper::is_layer1($mp3) )//V2,L1
            $whichcolumn=3;
        else if (Mp3fileHelper::is_mpeg20or25($mp3) && (Mp3fileHelper::is_layer2($mp3) || Mp3fileHelper::is_layer3($mp3)) )
    ...
    public static function samplelookup(&$mp3)
    {
        ...
        $whichcolumn=-1;
        if      (Mp3fileHelper::is_mpeg10($mp3))
            $whichcolumn=0;
        else if (Mp3fileHelper::is_mpeg20($mp3))
            $whichcolumn=1;
        else if (Mp3fileHelper::is_mpeg25($mp3))
            $whichcolumn=2;
